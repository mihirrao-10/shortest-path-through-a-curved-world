...
import {
  selectHeatInspectionFrames,
  type HeatInspectionFrame,
  type HeatInspectionState,
} from "./heat-inspection";
import type { SurfaceStart } from "./path-tracer";
import {
  buildPresetRouteComparison,
  buildRouteComparison,
  type RouteComparisonGeometry,
  type RouteComparisonMode,
} from "./route-comparison";
import type { TargetPresetName, WorldData } from "./world-data";

export type { HeatInspectionFrame, HeatInspectionState, RouteComparisonMode };
export type SurfaceDisplayMode =
  "surface" | "heat" | "distance" | "contours" | "mesh";
...
export interface RouteComparisonMetrics {
  ...
  edgeLength: number;
  heatLength: number;
  edgeReachesDestination: boolean;
  heatReachesDestination: boolean;
}

interface SceneOptions {
  reducedMotion: boolean;
  onExplorerPlaced?: (target: TargetPresetName | "custom") => void;
  onRouteMetricsChanged?: (metrics: RouteComparisonMetrics) => void;
}

const CAPTIONS = [
  "A closed handle-bearing surface generated by the C++ engine.",
  "The violet chord is shorter, but impossible to walk.",
  ...
  "20,480 faces here, with sparse factors reused across source queries.",
  "The route home, recovered from heat.",
] as const;

const ROUTE_CAPTIONS: Record<RouteComparisonMode, string> = {
  chord: "The violet chord measures ambient distance and leaves the surface.",
  edge: "The pale route is exact for the mesh graph and restricted to its edges.",
  heat: "The amber route numerically approximates a continuous surface geodesic.",
  compare:
    "The chord, edge route, and approximate surface route share the same ends.",
};
...
function pathTube(
  ...
  const segments = Math.max(80, Math.min(900, points.length * 4));
...
export class WorldScene {
  ...
  private readonly onExplorerPlaced?: (
    target: TargetPresetName | "custom",
  ) => void;
  private readonly onRouteMetricsChanged?: (
  ...
  private readonly cameraTarget = new THREE.Vector3(0.18, 0.1, 4.8);
  ...
  private routeGeometry: RouteComparisonGeometry | null = null;
  private routeComparisonMode: RouteComparisonMode | null = null;
  private surfaceDisplayMode: SurfaceDisplayMode = "surface";
  private activePreset: TargetPresetName | null = null;
  ...
  private heatInspectionFrame: number | null = null;
  ...
  constructor(
    ...
    this.onRouteMetricsChanged = options.onRouteMetricsChanged;
    ...
    this.canvas.dataset.surfaceMode = "surface";
    this.canvas.dataset.target = "exterior";
    ...
    this.world.rotation.y = -0.52;
    this.world.rotation.x = -0.32;
    this.world.scale.setScalar(
      window.matchMedia("(max-width: 820px)").matches ? 0.68 : 0.63,
    );
  ...
  get caption(): string {
    if (this.routeComparisonMode) {
      return ROUTE_CAPTIONS[this.routeComparisonMode];
    }
    return CAPTIONS[this.activeAct] ?? CAPTIONS[0];
  }

  getRouteComparisonMetrics(): RouteComparisonMetrics | null {
    if (!this.routeGeometry) return null;
    return {
      revision: this.routeRevision,
      chordLength: this.routeGeometry.chord.length,
      edgeLength: this.routeGeometry.edge.length,
      heatLength: this.routeGeometry.heat.length,
      edgeReachesDestination: this.routeGeometry.edge.reachesDestination,
      heatReachesDestination: this.routeGeometry.heat.reachesDestination,
    };
  }

  setRouteComparison(mode: RouteComparisonMode): void {
    this.routeComparisonMode = mode;
    this.canvas.dataset.routeMode = mode;
    this.applyRouteVisibility();
  }

  clearRouteComparison(): void {
    this.routeComparisonMode = null;
    delete this.canvas.dataset.routeMode;
    this.applyRouteVisibility();
  }

  getHeatInspectionFrames(): HeatInspectionFrame[] {
    return selectHeatInspectionFrames(this.data);
  }

  inspectHeatState(state: HeatInspectionState): HeatInspectionFrame {
    const selection = this.getHeatInspectionFrames().find(
      (frame) => frame.state === state,
    );
    if (!selection) throw new Error("Unknown heat inspection state");
    this.activeAct = 3;
    this.canvas.dataset.act = "3";
    this.heatInspectionFrame = selection.frameIndex;
    this.canvas.dataset.heatMode = "inspection";
    this.surfaceDisplayMode = "heat";
    this.canvas.dataset.surfaceMode = "heat";
    this.applyHeatFrame(selection.frameIndex);
    this.cameraTarget.copy(this.cameraForAct(3));
    return selection;
  }

  setSurfaceDisplay(mode: SurfaceDisplayMode): void {
    this.surfaceDisplayMode = mode;
    this.canvas.dataset.surfaceMode = mode;
    const wireMaterial = this.wireframe.material as THREE.MeshBasicMaterial;
    wireMaterial.opacity = mode === "mesh" ? 0.48 : 0;
    this.wireframe.visible = wireMaterial.opacity > 0;
    this.gradientLines.visible = this.activeAct === 4;
    if (mode === "surface" || mode === "mesh") {
    ...
    } else if (this.heatInspectionFrame !== null) {
      this.applyHeatFrame(this.heatInspectionFrame);
    } else {
      this.applyHeatFrame(Math.floor(this.data.heatFrameCount / 2));
    }
  }

  placeTargetPreset(name: TargetPresetName, announce = true): void {
    const preset = this.data.targetPresets[name];
    const face = this.findIncidentFace(preset.vertex);
    const point = vertexVector(this.data.positions, preset.vertex);
    const normal = vertexVector(this.data.normals, preset.vertex).normalize();
    this.activePreset = name;
    this.canvas.dataset.target = name;
    this.setExplorer({ face, point }, normal, announce, name);
  }

  resetInteractiveView(): void {
    this.heatStarted = false;
    this.heatInspectionFrame = null;
    this.canvas.dataset.heatMode = "idle";
    delete this.canvas.dataset.heatFrame;
    delete this.canvas.dataset.heatProgress;
    this.setSurfaceDisplay("surface");
    this.setRouteComparison("heat");
    this.placeTargetPreset("exterior", false);
    this.cameraTarget.copy(this.cameraForAct(this.activeAct));
  }
  ...
  setAct(act: number): void {
    ...
    this.applyRouteVisibility();
    this.setSurfaceDisplay(this.surfaceDisplayMode);
  ...
  replay(): void {
    ...
    this.actStartTime = performance.now();
    this.applyColors(this.baseColors);
    this.surfaceDisplayMode = "surface";
    this.canvas.dataset.surfaceMode = "surface";
    this.gradientLines.visible = false;
    this.applyRouteVisibility();
    this.camera.position.set(0.18, 0.1, 4.8);
  ...
  private buildBaseColors(): Float32Array {
    ...
    for (let vertex = 0; vertex < this.data.vertexCount; vertex += 1) {
      ...
      const height = THREE.MathUtils.clamp((radial - 0.45) / 1.2, 0, 1);
      const mix = THREE.MathUtils.clamp(
        0.08 + 0.66 * height + 0.14 * normalZ,
        0,
        1,
      );
  ...
  private applyHeatFrame(frameIndex: number): void {
    ...
    if (!frame) throw new Error("Heat frame index is out of range");
    for (let vertex = 0; vertex < this.data.vertexCount; vertex += 1) {
    ...
    this.colorAttribute.needsUpdate = true;
    this.canvas.dataset.heatFrame = String(frameIndex);
  }
  ...
  private findIncidentFace(vertex: number): number {
    for (let face = 0; face < this.data.faceCount; face += 1) {
      if (
        this.data.indices[3 * face] === vertex ||
        this.data.indices[3 * face + 1] === vertex ||
        this.data.indices[3 * face + 2] === vertex
      ) {
        return face;
      }
    }
    throw new Error("Target preset vertex has no incident face");
  }
  ...
  private setExplorer(
    ...
    normal: THREE.Vector3,
    userPlaced: boolean,
    target: TargetPresetName | "custom" = "custom",
  ): void {
    ...
    this.rebuildRoutes();
    if (userPlaced) this.onExplorerPlaced?.(target);
  }

  private rebuildRoutes(): void {
    if (!this.explorerStart) return;
    this.routeGeometry = this.activePreset
      ? buildPresetRouteComparison(this.data, this.activePreset)
      : buildRouteComparison(this.data, this.explorerStart);

    this.ambientLine?.removeFromParent();
    this.ambientLine?.geometry.dispose();
    (this.ambientLine?.material as THREE.Material | undefined)?.dispose();
    const ambientGeometry = new THREE.BufferGeometry().setFromPoints(
      this.routeGeometry.chord.points,
    );
    ...
    this.dijkstraLine?.geometry.dispose();
    (this.dijkstraLine?.material as THREE.Material | undefined)?.dispose();
    const dijkstraPoints = this.routeGeometry.edge.points.map((point) =>
      point.clone(),
    );
    ...
    this.geodesicPath?.geometry.dispose();
    (this.geodesicPath?.material as THREE.Material | undefined)?.dispose();
    this.geodesicPath = pathTube(
      this.routeGeometry.heat.points,
      0.009,
      0xffbd62,
    );

    this.routeRevision += 1;
    this.canvas.dataset.routeRevision = String(this.routeRevision);
    this.applyRouteVisibility();
    const metrics = this.getRouteComparisonMetrics();
    if (metrics) this.onRouteMetricsChanged?.(metrics);
  }

  private applyRouteVisibility(): void {
    const comparisonActive = this.routeComparisonMode !== null;
    const setOpacity = (
      object: THREE.Line | THREE.Mesh | null,
      opacity: number,
    ): void => {
      if (!object) return;
    ...
    const attach = (
      ...
      if (!object) return;
      if (visible) {
        if (!object.parent) this.world.add(object);
      } else {
    ...
    if (comparisonActive) {
      attach(this.ambientLine, true);
      attach(this.dijkstraLine, true);
      attach(this.geodesicPath, true);
      if (this.dijkstraLine) {
        const count = this.dijkstraLine.geometry.getAttribute("position").count;
        this.dijkstraLine.geometry.setDrawRange(0, count);
      }
      setOpacity(
        this.ambientLine,
        this.routeComparisonMode === "chord" ||
          this.routeComparisonMode === "compare"
          ? 0.98
          : 0.12,
      );
      setOpacity(
        this.dijkstraLine,
        this.routeComparisonMode === "edge" ||
          this.routeComparisonMode === "compare"
          ? 0.98
          : 0.12,
      );
    ...
    attach(this.ambientLine, this.activeAct === 1);
    attach(this.dijkstraLine, this.activeAct === 2);
    attach(this.geodesicPath, this.activeAct >= 4);
    setOpacity(this.ambientLine, this.activeAct === 1 ? 0.82 : 0);
    setOpacity(this.dijkstraLine, this.activeAct === 2 ? 0.9 : 0);
    setOpacity(this.geodesicPath, this.activeAct >= 4 ? 1 : 0);
  }

  private cameraForAct(act: number): THREE.Vector3 {
    const mobile = window.matchMedia("(max-width: 820px)").matches;
    const z = mobile
      ? [5.1, 5.0, 4.8, 4.95, 4.85, 4.9, 5.15, 4.9]
      : [4.8, 4.75, 4.45, 4.65, 4.5, 4.55, 4.85, 4.55];
  ...
  private readonly handlePointerUp = (event: PointerEvent): void => {
    ...
    this.canvas.dataset.target = "custom";
    this.setExplorer(
  ...
  private readonly handleKeyDown = (event: KeyboardEvent): void => {
    if (
      ...
      this.onExplorerPlaced?.("exterior");
    }
  };
