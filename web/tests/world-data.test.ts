...
describe("multi-world native export", () => {
  ...
  it("derives and validates topology for every genus without grid assumptions", () => {
    ...
    for (const genus of SUPPORTED_GENERA) {
      ...
      expect(metadata.heatDisplay.normalizationLogDynamicRangeDecades).toBe(14);
      expect(metadata.heatDisplay.allRouteStartsReached).toBe(true);
      expect(
        metadata.heatDisplay.minimumFinalRouteStartNormalizedHeat,
      ).toBeGreaterThanOrEqual(metadata.heatDisplay.routeStartReachThreshold);
      const finalFrame = data.heatFrames.at(-1)!;
      const routeStartHeat = metadata.routePresets.map((route) =>
        route.start.barycentric.reduce(
          (sum, weight, local) =>
            sum +
            weight *
              (finalFrame[data.indices[3 * route.start.face + local]!]! /
                65_535),
          0,
        ),
      );
      expect(Math.min(...routeStartHeat)).toBeGreaterThanOrEqual(
        metadata.heatDisplay.routeStartReachThreshold - 2 / 65_535,
      );
  ...
  it("rejects malformed topology and malformed native path ranges", () => {
    ...
    const falseCoverage = structuredClone(metadataValue) as {
      heatDisplay: { allRouteStartsReached: boolean };
    };
    falseCoverage.heatDisplay.allRouteStartsReached = false;
    expect(() => parseWorldMetadata(falseCoverage, data)).toThrow(
      /does not match|heat/i,
    );
